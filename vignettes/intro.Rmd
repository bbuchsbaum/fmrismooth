...
smoothed <- fmrismooth_default(noisy, robust = "none", auto_params = TRUE)
...
```{r}
mp_out <- fmrismooth_mppca_pipeline(
  noisy,
...
tv <- stv_denoise4d(noisy, lambda_s = 0.6, lambda_t = 0.2, iters = 20L)

# Robust variant using Huber or Tukey data terms
rob <- stv_robust4d(noisy, loss = "huber", iters = 20L)
...
vst_out <- vst_wrap(
  noisy,
  denoise_fun = function(z) fast_bilateral_lattice4d(z, sigma_sp = 2.0, sigma_t = 0.4, sigma_r = 8)
)
...
```{r}
rec <- recommend_params(noisy, tr = 2.0, target_fwhm_mm = 5)
str(rec)

out <- fmrismooth_default(noisy, robust = "none", auto_params = FALSE)
```
